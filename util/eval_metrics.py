...
import sys
from util.rankResults import visualize_ranked_results
...
def make_results_visaul(qf, gf, lqf, lgf, q_pids, g_pids, q_camids, g_camids, targetmodel, ak_typ, attr_matrix=None, dataset_name=None, attr=None):
  qf, gf = featureNormalization(qf, gf, targetmodel)
  m, n = qf.size(0), gf.size(0)
  distmat = torch.pow(qf, 2).sum(dim=1, keepdim=True).expand(m, n) + torch.pow(gf, 2).sum(dim=1, keepdim=True).expand(n, m).t()
  distmat.addmm_(1, -2, qf, gf.t())
...
def featureNormalization(qf, gf, targetmodel):
  if targetmodel in ['aligned', 'densenet121', 'hacnn', 'mudeep', 'ide', 'cam', 'lsro', 'hhl', 'spgan','bot','vit','vit_norm','transreid']:
