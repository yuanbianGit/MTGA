...
class Market1501(object):
    ...
    def __init__(self, root='data', **kwargs):
        ...
        self.mask_dir = osp.join(self.dataset_dir, 'bounding_box_train_mask')
        ...
        mask, num_mask_pids, num_mask_imgs = self._process_dir(self.mask_dir, relabel=False)
        ...
        self.num_mask_pids = num_mask_pids
...
class Market1501_my(object):
    ...
    def __init__(self, root='data', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'bounding_box_train')
        self.query_dir = osp.join(self.dataset_dir, 'query_my')
        self.gallery_dir = osp.join(self.dataset_dir, 'bounding_box_test_my')
        self.mask_dir = osp.join(self.dataset_dir, 'bounding_box_train_mask')
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        for img_path in img_paths:
            pid, camid = map(int, pattern.search(img_path).groups())
            if pid == -1: continue  # junk images are just ignored
            assert 0 <= pid <= 1510  # pid == 0 means background
            assert 1 <= camid <= 6
            camid -= 1  # index starts from 0
            pid_raw = pid
            if relabel:
...
class CUHK03(object):
    ...
    def __init__(self, root='data', split_id=0, cuhk03_labeled=False, cuhk03_classic_split=False, **kwargs):
        self.dataset_dir = osp.join(root, 'cuhk03')
...
class DukeMTMCreID(object):
    ...
    def __init__(self, root='data', **kwargs):
        ...
        self.mask_dir = osp.join(self.dataset_dir, 'DukeMTMC-reID/bounding_box_train_mask')
...
class SYSU(object):
    ...
    def __init__(self, root='data', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'SYSU-30k-released/sysu_train_set_all')
        self._check_before_run()
        train, num_train_pids, num_train_imgs = self._process_dir(self.train_dir, relabel=True)
        self.train = train
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        img_paths_file = '/data4/by/reid/dataSet/SYSU-30k-released/meta/train.txt'
        # pattern = re.compile(r'([-\d]+)_c(\d)')
        #
        # pid_container = set()
        # for img_path in img_paths:
        ...
        with open(img_paths_file, 'r') as f:
            ...
            for line in f:
                # 去除每行的换行符，并将路径添加到列表中
                img_path = osp.join(self.train_dir, line.strip())
                dataset.append((img_path, pid, camid, pid_raw))


        num_pids = len(dataset)
        num_imgs = len(dataset)
        return dataset, num_pids, num_imgs
class MSMT17(object):
    """
