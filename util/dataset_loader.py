...
class ImageDataset(Dataset):
    ...
    def __init__(self, dataset, transform=None):
        ...
        self.mask_trans = T.Compose([T.Resize((256,128), interpolation=3), T.ToTensor()])
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        if img_path.__contains__("train"):

            img = read_image(img_path)
            mask_img_path = img_path.replace("train","train_mask")
            img_mask = read_image(mask_img_path)
            if self.transform is not None:
                img = self.transform(img)
                img_mask = self.mask_trans(img_mask)
            # return img, pid, img_mask, pid_raw
            return img, pid, img_mask, camid
